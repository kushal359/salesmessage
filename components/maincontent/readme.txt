...
import { Container, Grid, Skeleton, Card, Group, Text, Stack, Select } from '@mantine/core'
...
export function MainContent() {
  ...
  useEffect(()=>{
    ...
    const results = await Promise.allSettled(
      conversation_ids.map((id) => fetchConversation(id))
    );
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled" && result.value) {
        const id = conversation_ids[index];
      ...
      <Grid>
        ...
        {statsConfig.map((item) => {
          ...
          return (
            <Grid.Col
              key={item.key}
              className={classes.fourboxescommon}
        ...
        <Grid.Col span={12}>
          <Grid>
            {memberStats.map((member) => (
              <Grid.Col key={member.id} span={{ base: 12, md: 6, lg: 3 }}>
                <Card shadow="sm" padding="lg" withBorder>
                  <Stack>
                    <Text fw={700}>{member.name}</Text>
                    <Text size="sm">Inbound Msg: {member.inboundMsg}</Text>
                    <Text size="sm">Outbound Msg: {member.outboundMsg}</Text>
                    <Text size="sm">Inbound Calls: {member.inboundCall}</Text>
                    <Text size="sm">Outbound Calls: {member.outboundCall}</Text>
                  </Stack>
                </Card>
              </Grid.Col>
            ))}
  ...
  try {
    while (hasMore) {
      const res: Response = await fetch(
        `/api/webhook/message?start=${start}&end=${end}&limit=500&cursor=${cursor ?? ""}`
      );
